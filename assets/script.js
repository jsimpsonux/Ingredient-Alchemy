            ...
            for (let i=0; i<recipeIdsArray.length; i++) {
                ...
                .then((data) => {
                    ...
                    for (let i=0; i < recipesList.length; i++) {
                        ...
                        // Appending recipe title
                        let titleElement = $('<h5>').addClass("card-title").text(recipeTitle);
                        cardBody.append(titleElement);
    
                        // Appending recipe image
                        let imageElement = $('<img>').addClass("card-img-top").attr("src", recipeImage).attr("alt", "Recipe Image");
                        cardBody.append(imageElement);
    
                        // Appending required ingredients list for the current recipe
                        const requiredIngredients = [];
                        for (let j=0; j < data.extendedIngredients.length; j++) {
                            const ingredientName = data.extendedIngredients[j].originalName;
                            requiredIngredients.push(ingredientName);
                            let ingredientContentHtml = $('<p>').text(ingredientName);
                            cardBody.append(ingredientContentHtml);
                        }
    
                        // Append the card body to the recipe container
